...
func resource(device: Device) -> Responder {
    ...
    return { context, request in
        ...
        DispatchQueue.main.sync {
            response = resource(device: device, request: request)
        }
...
func resource(device: Device, request: HTTPRequest) -> HTTPResponse {
    ...
    // {"image-height":240,"resource-type":"image","image-width":320,"aid":2}
    logger.info("received body: \(String(decoding: body, as: UTF8.self))")
    logger.info("identified accessory: \(accessory.aid)")

    //let outBody = try JSONEncoder().encode(Protocol.CharacteristicContainer(characteristics: responses))
    return HTTPResponse(status: .ok,
                        headers: HTTPHeaders([
                            ("Content-Type", "image/jpeg")
                        ]),
                        body: "i don't have an image yet...")
